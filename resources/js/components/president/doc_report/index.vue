...
const get_SumDocument = async () => {
    try {
        let response = await axios.get(`/api/president_get_docSum/${userId}`);
        doc_sum.value = response.data.doc_sum;
    } catch (error) {
        console.error("Error fetching doc_sum:", error);
    }
};
...
const getSubject = async (page = 1) => {
    try {
        let response = await axios.get(
            `/api/president_get_subjectAll_term/${User_pId.value}/${currentTerm.value}/?page=${page}`
        );
...
<template>
        ...
        <div class="col-lg-12">
            <div class="card p-1">
                <div class="card-body pt-3">
                    <h5 class="mt-lg-0 mt-4 pb-0 mb-0 text-dark text-gradient">
                        รายงานการจัดส่ง มคอ.
                    </h5>
                    <p class="mt-0">รายงานการจัดส่ง มคอ</p>
                            ...
                            <div class="row">
                                <div class="col-3 py-3">
                                    <div class="d-flex mb-2">
                                        ...
                                        <p
                                            class="text-xs mt-1 mb-0 font-weight-bold"
                                        >
                                            Subject
                                        </p>
                                    </div>
            ...
            <div class="card mt-3">
                ...
                <div class="table-responsive">
                    <table class="table align-items-center mb-0 table-hover">
                        <thead>
                            <tr>
                                <th class="text-dark text-sm td-doc th-1">
                                    รายวิชา
                                </th>

                                <th
                                    class="text-start text-dark text-sm ms-0 ps-2"
                                >
                                    อาจารย์
                                </th>
                        ...
                        <tbody>
                            <tr
                                v-for="items in subjects.data"
                                :key="items.ts_id"
                            >
                                <td>
                                    <div class="d-flex px-3">
                                        <div
                                            class="d-flex flex-column justify-content-center py-1"
                                        >
                                            <label class="mb-0 text-sm">
                                                {{ items.subjects.s_name }}
                                            </label>
                                            <p
                                ...
                                <td>
                                    <label
                                        for=""
                                        class="text-sm fw-normal ps-0 ms-0 text-muted"
                                    >
                                        {{ items.subjects.s_credit }}</label
                                    >
                                </td>
                                <td class="align-middle text-center text-sm">
                                    <div v-if="items.docfile.length > 0">
                                        <div
                                            v-for="docfile in items.docfile"
                                            :key="docfile.df_id"
                                        >
                                            <div
                                ...
                                <td class="align-middle text-center">
                                    <div>
                                        <button
                                            class="btn bg-gradient-dark shadow btn-sm mb-0 fw-normal"
                                            @click="Subject_detail(items.ts_id)"
                                        >
                                            ดูรายละเอียด
                                        </button>
                                    </div>
                ...
                <div class="container mt-2">
                    <Bootstrap5Pagination
                        :data="subjects"
                        @pagination-change-page="getSubject"
                    />
                </div>
            </div>
        </div>
    ...
    <div class="col-lg-6 col-sm-12 col-md-12">
        <Pie :options="chartOptions" :data="chartData" class="mt-4" />
    </div>
</template>
