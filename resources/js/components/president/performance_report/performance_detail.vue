...
const getPerformance_file = async () => {
    try {
        let response = await axios.get(
            `/api/user_get_performance_file/${props.id}`
        );
...
const get_teacher = async () => {
    try {
        let response = await axios.get(`/api/user_get_teacher/${props.id}`);
...
const PerformanceDetail = (id) => {
    router.push(`/president/performance_report/performance_view/${id}`);
};
let input = ref("");
const fruits = ["apple", "banana", "orange"];
function filteredList() {
    return performance.value.filter(
        (item) =>
            item.pf_name.toLowerCase().includes(input.value.toLowerCase()) ||
            item.pf_year.toLowerCase().includes(input.value.toLowerCase())
    );
}
onMounted(async () => {
            ...
            <div class="card h-100">
                <div class="card-body p-3">
                            ...
                            <div class="dropdown">
                                ...
                                >
                                    <a
                                        class="dropdown-item"
                                        href="javascript:;"
                                        @click="PerformanceDetail(item.pf_id)"
                                        >ดูรายละเอียด</a
                                    >
                                </div>
                            </div>
                    ...
                    <p class="text-sm mb-0">
                        รายละเอียด : {{ item.pf_detail }}
                    </p>
                    <p class="text-sm mb-0">วันที่ : {{ item.pf_date }}</p>
                    <p class="text-sm mb-3">
                        ปีที่เผยแพร่ : {{ item.pf_year }}
                    </p>
                    <p class="text-sm fw-bold">
                        ไฟล์เอกสาร : {{ item.performance_file.length }} ไฟล์
                    </p>
